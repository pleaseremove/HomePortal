...
class Utils extends My_Controller {
	...
	function verify(){
		$csv_data = file_get_contents('Statement Download 2015-Feb-14 11-25-21.csv');
		$csv_array = explode("\n",$csv_data);
		...
		foreach($csv_array as $k => $line){
			if($k > 4 && !empty($line)){
				...
				if($debit!=''){
					...
					$search = $this->db->query('SELECT * FROM v_money_transactions WHERE deleted = 0 AND `date` BETWEEN "'.$start_date.'" AND "'.$end_date.'" AND trans_type = -1 AND account_id = '.$this->account_id.' AND amount = "'.$debit.'"');
					...
					$search = $this->db->query('SELECT * FROM v_money_transactions WHERE deleted = 0 AND `date` BETWEEN "'.$start_date.'" AND "'.$end_date.'" AND trans_type = 1 AND account_id = '.$this->account_id.' AND amount = "'.$credit.'"');
